...
**Pattern**: When adding a new entry type to an existing log table, use a sentinel URI prefix (e.g. `manual://<uuid>`) on the existing `photo_uri` column instead of creating a second table. **Reason**: a second table requires a second list endpoint, second schema type, and every downstream consumer (score engine, RAG, UI) must be taught about both. The sentinel prefix keeps all log entries queryable in one place and is detectable in O(1) without schema changes. **Solution**: store `photo_uri = f"manual://{uuid4()}"` and `classification = "manual"`. Readers guard with `photo_uri.startswith("manual://")` before rendering an image.

**Pattern**: Today page mixes two invalidation strategies — optimistic local state for inline protocol actions (complete/skip/reorder) and `router.refresh()` for BottomSheet submissions (sleep/water/workout/meal/check-in). **Reason**: protocol row state (checked/skipped/order) is already held in client state and only visible in the protocol list, so a full server round-trip is wasteful. Sheet submissions affect macro rings, vitality sub-scores, and the weekly card which are all server-rendered — they need a real refetch. **Solution**: inline protocol mutations update `useState` optimistically and roll back on error; sheet `onSubmitted` callbacks call `router.refresh()` to trigger a server component re-render of the whole Today tree.

**Pattern**: `PatientData` dataclass fields that are new optional lists (`daily_logs`, `meal_logs`) must use `dataclasses.field(default_factory=list)`, not a bare `= None` or `= []` default. **Reason**: a mutable default (`= []`) is shared across all instances (Python dataclass gotcha), and `= None` forces every call site to guard against `None` before iterating. `default_factory=list` gives each instance its own empty list and keeps the type `list[T]` (not `list[T] | None`), so existing adapters that don't supply the field remain valid without changes. **Solution**: declare optional list fields as `field_name: list[X] = dataclasses.field(default_factory=list)` in `PatientData`.

**Pattern**: The Vitality Outlook horizon ordering is **monotonically non-decreasing** (3m ≤ 6m ≤ 12m) in the capped-ceiling model — opposite of any decay model where shorter horizons project higher. **Reason**: in the gap-closing formula `current + gap × adherence × streak_mult × horizon_factor(h)`, `horizon_factor` grows with `h` (`{3: 0.25, 6: 0.50, 12: 0.70}`), so longer horizons always project equal-or-higher values. **Solution**: when writing tests or UI copy for the outlook curve, expect the 12-month bar to be the tallest. Assert `projections[3] <= projections[6] <= projections[12]` in all outlook tests.
