...
`[ref:ehr_record.<id>]` where `<id>` is the numeric record identifier
(e.g. `[ref:ehr_record.42]`). If a fact is not in the provided records, say:
...
2. Every factual statement must cite its source record: `[ref:ehr_record.<id>]`
   where `<id>` is the numeric record identifier (e.g. `[ref:ehr_record.42]`).
...
Use inline citations after each factual claim, using the exact tag format
`[ref:ehr_record.<id>]` where `<id>` is the numeric record ID:
> "Your most recent HbA1c was 5.4% [ref:ehr_record.42], recorded on 2025-11-03."

Do NOT use any other format such as `[ref:rec-XXXXX]` or `[ref:<name>]`.
Only `[ref:ehr_record.<numeric_id>]` is recognised by the citation parser.
